...
int main() {
    ...
    int* tabc;
    tabc = calloc( MAX, sizeof( int )); // allocate 5 multiplied by the size of int
    printString( "\nThe contents of array tabc: " );


    //Walking through the array using the [] notation
    for ( int i = 0; i < MAX; i++ )  {
    ...
    //Walking through the array using pointer notation
    for ( int i = 0; i < MAX; i++ )  {
    ...
    //Walking through the array using a variable pointer
    for ( int i = 0; i < MAX; i++ ) {
        *p = i * i;
        p++;
    }
